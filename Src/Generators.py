...
choice_text = f"\n{DARK_GRAY}{localized_text('apply_promo_after_generate')}{CYAN}\n{localized_text('yes_enter')}: ".lower()
...
def generate_promocodes(prefix='', apply_promo=False):
    ...
    if count == '':
        count = 1
        print(localized_text('count_not_specified'))
...
async def genetare_for_all_games():
    ...
    count = input(localized_text('count_promocodes_to_generate_all_games'))
    if count == '':
        count = 1
        print(localized_text('count_not_specified'))
