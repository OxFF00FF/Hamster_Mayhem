...
## Обновление
 -  Добавлен перевод интерфейса на английский язык.

 -  В главном меню теперь есть настройки (s)
 
 - Чтобы поменять язык нужно в настройках выбрать пункт 4 и перезайти
   
 - Чтобы вам было не так скучно ждать генерацию промокодов,
   была добавлена возможность изменять индикатор загрузки 😉

   Команда `spinner_<name>` в главном меню выбирает индикатор.

   Команда `spinner_list`, показывает все доступные варианты имён.

   Команда `spinner_default`, установит индикатор по умолчанию
